...
class WritingGoal(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='writing_goals')
    ...
    end_date = models.DateField(null=True, blank=True, 
                               help_text="Leave blank for ongoing goals")
    active = models.BooleanField(default=True)
    notes = models.TextField(blank=True, 
                           help_text="Why did you set this goal? What are you working towards?")
    ...
    def progress_percentage(self):
        ...
        if self.goal_type == 'time':
            total_minutes = sessions.aggregate(models.Sum('minutes_spent'))['minutes_spent__sum'] or 0
            return min(int((total_minutes / self.target_value) * 100), 100)
        elif self.goal_type == 'words':
            total_words = sessions.aggregate(models.Sum('word_count'))['word_count__sum'] or 0
...
class WritingSession(models.Model):
    ...
    word_count = models.PositiveIntegerField(
        default=0,
        help_text="Approximately how many words did you write? (Optional)"
    )
    ...
    notes = models.TextField(
        blank=True,
        help_text="What did you write about? How did it go?"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
